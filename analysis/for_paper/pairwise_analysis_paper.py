...
def process_csv_folder(folder_path):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))  # set a wider figure size

    x = np.arange(1, 20)
    plt.plot(x, list(win_diffs.values()))
    plt.plot([1, 19], [0, 0], 'k--', lw=1) 
    
    plt.xlabel("Number of "+master_trader+" traders",fontsize=14)
    plt.tick_params(axis='x', top=False)  
   
    
    plt.ylabel("Wins Difference",fontsize=14)
    plt.xlim(1, 19)  
    plt.ylim(-1000, 1000)
    plt.yticks([-1000, -500, 0, 500, 1000])
    plt.grid(axis='y', linestyle='-', alpha=0.7)  
    plt.box(False) 
    plt.savefig("half_second_batch_interval_varying_EQ/"+comparison+".pdf")
    #plt.show()
    plt.clf()
